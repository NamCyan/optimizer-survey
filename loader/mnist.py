...
def load(valid_rate, batch_size):
    ...
    trainset= datasets.MNIST(savefolder,train=True,download=True,
                                    transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean[0],std[0])]))
    test= datasets.MNIST(savefolder,train=False,download=True,
                                    transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean[0],std[0])]))
